...
const filterOrdersByType = (orders: Order[], screenType: 'today' | 'pending' | 'preparing') => {
...
const OrderScreen: React.FC<OrderScreenProps> = ({ type, title }) => {
  ...
  // Function to fetch orders from AsyncStorage
  ...
  const fetchOrders = async () => {
    try {
      ...
      const filteredLocalOrders = filterOrdersByType(localOrders, type);
      ...
      // Filter orders based on screen type
      const filteredOrders = filterOrdersByType(sortedOrders, type);

      // Update local storage with all orders
      await saveLocalOrders(sortedOrders);
  ...
  const renderOrder = ({ item: order }: { item: Order }) => (
    <TouchableOpacity 
      onPress={() => router.push(`/screens/OrderDetail?order_id=${order.order_id || order.id}`)}
    ...
    >
      ...
      <View style={styles.orderHeader}>
        <View style={styles.orderInfo}>
          <Text style={[styles.orderId, { color: isDark ? '#FFFFFF' : '#1F2937' }]}>
        ...
        <View style={styles.orderType}>
          <MaterialCommunityIcons 
            name={
              order.order_type === 'Dine In' ? 'food-fork-drink' : 
              order.order_type === 'Take Away' ? 'shopping-outline' : 
      ...
      <View style={styles.orderDetails}>
        {order.order_type === 'Dine In' && (
          <View style={styles.detailItem}>
            <MaterialCommunityIcons name="table-furniture" size={18} color={isDark ? '#9CA3AF' : '#6B7280'} />
            <Text style={styles.detailText}>Table {order.table_num}</Text>
          </View>
        )}
        <View style={styles.detailItem}>
          <MaterialCommunityIcons name="food" size={18} color={isDark ? '#9CA3AF' : '#6B7280'} />
          <Text style={styles.detailText}>{order.count_item} items</Text>
        </View>
        <View style={styles.detailItem}>
          <MaterialCommunityIcons name="cash" size={18} color={isDark ? '#9CA3AF' : '#6B7280'} />
