...
type RouteNames = '/screens/TodayOrders' | '/screens/revenue' | '/screens/pendingOrders' | '/screens/PreparingOrders';
...
const Dashboard = () => {
  ...
  const fetchDashboardStats = async () => {
    try {
      const response = await axios.get(`/dashboard/stats/${user?.vendor_id}`)
  ...
  const StatCard = ({ title, value, icon, color }: any) => (
    <TouchableOpacity 
      style={[styles.statCard, { backgroundColor: isDark ? COLORS.dark : COLORS.white }]}
      onPress={() => {
        if (title === "Today's Revenue") return;
        const route: RouteNames = 
          title === "Today's Orders" ? '/screens/TodayOrders' :
          title === "Pending Orders" ? '/screens/pendingOrders' :
    ...
    >
      ...
      <View>
        ...
        <Text style={[styles.statTitle, { color: isDark ? COLORS.gray : '#6B7280' }]}>
          {title}
        </Text>
      </View>
    </TouchableOpacity>
  ...
  return (
    <ThemedView style={styles.container}>
      ...
      >
        ...
        <View style={[styles.section, { backgroundColor: isDark ? COLORS.dark : COLORS.white }]}>
          ...
          {stats.popularItems.map((item, index) => (
            <View key={index} style={styles.popularItem}>
              ...
              <Text style={[styles.itemName, { color: isDark ? COLORS.white : COLORS.dark }]}>
                {item.name}
              </Text>
            ...
            <View key={index} style={styles.recentOrder}>
              <View style={styles.orderInfo}>
                <MaterialCommunityIcons 
                  name={
                    order.type === 'Dine In' ? 'food-fork-drink' : 
                    order.type === 'Take Away' ? 'shopping-outline' : 
                ...
                />
                <View>
                  <Text style={[styles.orderType, { color: isDark ? COLORS.white : COLORS.dark }]}>
                    {order.type}
                  </Text>
                  {order.table && (
              ...
              <View style={styles.orderStatus}>
                ...
                <View style={[
                  styles.statusBadge, 
                  { backgroundColor: COLORS.statusColors[order.status]?.concat('15') || COLORS.primary + '15' }
                ]}>
                  <Text style={[
                    styles.statusText,
                    { color: COLORS.statusColors[order.status] || COLORS.primary }
